...
Public Class frmTermProgs
    ...
    Private Sub EnableMenu()
        ...
        Try
            Select Case Userx
                ...
                Case "USER Department"
                    ...
                    If (UserAccessControls And (2 ^ 0)) = 0 Then Menu_Courses.Enabled = False Else Menu_Courses.Enabled = True
                    If (UserAccessControls And (2 ^ 1)) = 0 Then Menu_Staff.Enabled = False Else Menu_Staff.Enabled = True
                    If (UserAccessControls And (2 ^ 1)) = 0 Then Menu_Tech.Enabled = False Else Menu_Tech.Enabled = True
                    If (UserAccessControls And (2 ^ 3)) = 0 Then Menu_ReProgram_ThisEnteryTerm.Enabled = False Else Menu_ReProgram_ThisEnteryTerm.Enabled = True
                    If (UserAccessControls And (2 ^ 3)) = 0 Then Menu_ReProgram_ThisEnteryTerm_inclStaff.Enabled = False Else Menu_ReProgram_ThisEnteryTerm_inclStaff.Enabled = True
                    If (UserAccessControls And (2 ^ 4)) = 0 Then Menu_ChangePass.Enabled = False Else Menu_ChangePass.Enabled = True
                    If (UserAccessControls And (2 ^ 4)) = 0 Then ContextMenuGrid4.Enabled = False Else ContextMenuGrid4.Enabled = True
        ...
        If (UserAccessControls And (2 ^ 2)) = 0 Then
            lbl_UserInactiveClass.Text = "کلا س بندي : غير فعال"
            lbl_UserInactiveClass.ForeColor = Color.Red
        Else
            lbl_UserInactiveClass.Text = "کلا س بندي : فعال"
            lbl_UserInactiveClass.ForeColor = Color.Blue
        End If

        If (UserAccessControls And (2 ^ 4)) = 0 Then
            lbl_UserInactiveProg.Text = "برنامه ريزي : غير فعال"
            lbl_UserInactiveProg.ForeColor = Color.Red
        Else
            lbl_UserInactiveProg.Text = "برنامه ريزي : فعال"
            lbl_UserInactiveProg.ForeColor = Color.Blue
        End If

        Select Case Userx
            Case "USER Faculty" : lbl_UserType.Text = "کاربر دانشکده"
    ...
    Private Sub DrawFreeTimeTable()
        Try
            ...
            Next d
            PrintLine(1, "</table>")
            PrintLine(1, "<p style='font-family:tahoma; font-size:12px'><br></p>")
        Catch ex As Exception
            MsgBox("Error in drawing free time table" & vbCrLf & ex.ToString, vbOKOnly, "nexterm")
        End Try
            ...
            For intTime As Integer = 0 To 7 ' for each time of day
                For intDay As Integer = 0 To 5
                    If (DS.Tables("tblTermProgs").Rows(intThisCourse).Item(intDay + 10) And (2 ^ intTime)) = (2 ^ intTime) Then ' if time[2^0] is set //item(10):SAT1
                        ...
                        If intTimeFlag(intDay, intTime) > 1 Then strTadakhol = strTadakhol & "<tr><td>" & strDay(intDay) & "</td><td>" & strTime(intTime) & "</td><td>" & DS.Tables("tblTermProgs").Rows(intThisCourse).Item(3) & "</td><td>" & DS.Tables("tblTermProgs").Rows(intThisCourse).Item(5) & "</td></tr>" & vbCrLf : TadakholExists = True
                    End If
    ...
    Private Sub Grid4_CellDblClick(sender As Object, e As DataGridViewCellEventArgs) Handles Grid4.CellDoubleClick
        ...
        Select Case c'SELECT BASED ON GRID.COLUMN
            ...
            Case 17, 25 'Room1, Room2
                If (UserAccessControls And (2 ^ 2)) = 0 Then MsgBox("قابليت (کلاس بندي) اکنون براي شما غير فعال است", vbInformation, "تنظيمات نکسترم") : Exit Sub
                Try
    ...
    'Exam Date/Time
    Private Sub lblExamDate_Click(sender As Object, e As EventArgs) Handles lblExamDate.Click
        Dim r As Integer = Grid4.SelectedCells(0).RowIndex 'count from 0
        Dim c As Integer = Grid4.SelectedCells(0).ColumnIndex 'count from 0
        If r < 0 Then Exit Sub
        If Grid4.Item(c, r).Value Is Nothing OrElse IsDBNull(Grid4.Item(c, r).Value) Then Exit Sub
    ...
    Private Sub Menu_ChangePass_Click(sender As Object, e As EventArgs) Handles Menu_ChangePass.Click
        If (Userx = "USER Faculty") Or (intUser = 0) Then 'MsgBox("کاربر دانشکده (ادمين) براي تغيير پسورد به بخش تنظيمات مراجعه کنيد", vbOKOnly, "نکسترم")
            ...
            Exit Sub
        End If
        If (UserAccessControls And (2 ^ 4)) = 0 Then MsgBox("قابليت (تغيير کلمه عبور) اکنون براي شما غير فعال است", vbInformation, "تنظيمات نکسترم") : Exit Sub
    ...
    Private Sub Menu_Departments_Click(sender As Object, e As EventArgs) Handles Menu_Departments.Click
        If (UserAccessControls And (2 ^ 4)) = 0 Then MsgBox("دسترسي به منابع اکنون براي شما غير فعال است", vbInformation, "تنظيمات نکسترم") : Exit Sub
        frmDepts.ShowDialog()
        ClrForm()
        EnableMenu()
            ...
            If (ReportSettings And 4) = 0 Then '//Days of Week in ROWS:0 (for Dr. RoshanZamir)
                ...
                For intday As Integer = 0 To 5
                    ...
                    For intTime As Integer = 0 To 7
                        ...
                        For i As Integer = 0 To DS.Tables("tblAllProgs").Rows.Count - 1
                            If ((DS.Tables("tblAllProgs").Rows(i).Item(intday + 10) And (2 ^ intTime)) = (2 ^ intTime)) Or ((DS.Tables("tblAllProgs").Rows(i).Item(intday + 18) And (2 ^ intTime)) = (2 ^ intTime)) Then
                                If (ReportSettings And 8) = 8 Then PrintLine(1, DS.Tables("tblAllProgs").Rows(i).Item(3), "<br>")              ' 3 :CourseName
                                PrintLine(1, DS.Tables("tblAllProgs").Rows(i).Item(7), "<br>")                                                 ' 7 :Staff
                                If (ReportSettings And 16) = 16 Then PrintLine(1, DS.Tables("tblAllProgs").Rows(i).Item(2), "<br>")            ' 2 :CourseNumber
                                If (ReportSettings And 32) = 32 Then PrintLine(1, " گروه ", DS.Tables("tblAllProgs").Rows(i).Item(5), "<br>")  ' 5 :Group
                                If (ReportSettings And 64) = 64 Then PrintLine(1, DS.Tables("tblAllProgs").Rows(i).Item(27), "<br>")           ' 27:Exam
                            End If
                        Next i
                        PrintLine(1, "</td>")
                    Next intTime
...
lblx2:
    ...
    Private Sub txtCMD0_TextChanged(sender As Object, e As EventArgs) Handles txtCMD0.TextChanged
        If txtCMD0.Text = "" Then Exit Sub
        Try
            Select Case cmdLineStatus0
                Case 0 'Ready for Commands
                    Select Case txtCMD0.Text.ToLower
                        ...
                        Case "sett"
                            txtCMD0.Text = "" : cmdLineStatus0 = 0
                            If Userx = "USER Faculty" Or intUser = 0 Then Menu_Settings_Click(sender, e)
                        Case "help"
                        ...
                        Case "cmd2 with parameter"
                            ...
                            '//txtCMD0.Text = "default parameters"
                            txtCMD0.SelectionStart = Len(txtCMD0.Text)
                        Case "cmd3 with parameter"
                ...
                Case 2 '---------------------------------------------------------- input parameter for cmd2
                    If Mid(txtCMD0.Text, Len(txtCMD0.Text), 1) = "#" Then
                        Dim sttng As String = Mid(txtCMD0.Text, 1, Len(txtCMD0.Text) - 1)
                        '//do cmd with parameter: sttng
                        '//WriteLOG(n)
                        cmdLineStatus0 = 0 'reset, ready for commands
                        txtCMD0.Text = ""
                    End If
                Case 3 '---------------------------------------------------------- input parameter for cmd3
                    If Mid(txtCMD0.Text, Len(txtCMD0.Text), 1) = "#" Then
                        Dim sttng As String = Mid(txtCMD0.Text, 1, Len(txtCMD0.Text) - 1)
                        '//do cmd with parameter: sttng
                        '//WriteLOG(n)
    ...
    Private Sub cmdHelp()
        ...
        PrintLine(1, "<hr>")
        PrintLine(1, "<p style='color:blue;font-family:tahoma; font-size:14px'>راهنماي خط فرمان در پايين صفحه اصلي نکسترم<br></p>")
        ...
        'Header
        PrintLine(1, "<tr><th></th><th>فرمان</th><th>عمل</th</tr>")
        'Rows
        PrintLine(1, "<tr><td>کارشناس آموزش</td>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> rep ent   </td> <td> گزارش برنامه ورودي ها        </td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> rep class </td> <td> گزارش برتامه کلاس ها          </td></tr>")
        ...
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> term    </td> <td> ويرايش ليست ترم ها - احتياط    </td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> tech    </td> <td> ويرايش ليست کارشناسان          </td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> class   </td> <td> ويرايش ليست کلاس ها             </td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> user    </td> <td> تغيير کاربر                    </td></tr>")
        PrintLine(1, "<tr><td>مديران گروه ها</td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> pass    </td> <td> تغيير پسورد                    </td></tr>")
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> temp    </td> <td> طراحي الگوي برنامه ريزي        </td></tr>")
        ...
        PrintLine(1, "<tr><td></td> <td style= 'Text-Align:Center'> about   </td> <td> صفحه آبي معرفي نکسترم          </td></tr>")
    ...
    Private Sub WriteLOG(intActivity As Integer)
        If boolLog = True Then
            ...
            Select Case intActivity
                ...
                Case 11 : strLog = "usr.pwd?" & strDepartmentPass
                Case 12 : strLog = "rev.staff.prgs"
                Case 13 : strLog = "rprt.ent.prgs"
                Case 14 : strLog = "rprt.class.prgs"
                Case 15 : strLog = "rprt.staff.prgs"
    ...
    Private Sub frmTermProgs_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        If e.CloseReason = CloseReason.UserClosing Then
            e.Cancel = True
            MsgBox("براي خروج از برنامه از منو استفاده کنيد ", vbOKOnly, "نکسنرم")
        End If
    End Sub
